...
## Anyone you worked with on this project: Joseph Stempel
...
def find_urls(s):
    urls = re.findall('http[s]?://\S+\.\S{2,}', s)  #find all occurences that contain http:// or https://, and at least 2 characters following a "."
    return urls
...
def get_umsi_data():
    ...
    page = 0
    #get the html associated with each page of the directory
    while (page < 13):
        names = []
        jobs = []

        #access the class_
        info = soup.find_all('div', class_ = "field-item even")
        #create a list of where every other item is a name and every other is a title to get rid of empty strings
        lst = []
        for item in info:
            if item.string != None:
        ...
        for index in range(len(lst)):
            if (index % 2) != 0:        #if the index is divisable by 2 (even), add to list of jobs
                jobs.append(lst[index])
            else:                       #if it's odd, add it to names
                names.append(lst[index])

        for n in range(len(names)):     #add each pair of names and titles to dictionary
            umsi_titles[names[n]] = jobs[n]

        page += 1                   #go to next page
        url = base_url + "&page=" + str(page)     #new url based on page number
        r = requests.get(url, headers={'User-Agent': 'SI_CLASS'})           #access new url
        soup = BeautifulSoup(r.content, 'html.parser')         #create new soup for updated url
